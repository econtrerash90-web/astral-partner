    ...
    >
      ...
      <div className="glass-card p-6 sm:p-8">
        ...
        <div className="bg-muted/30 p-5 rounded-xl border border-border mb-4">
          <p className="text-muted-foreground text-xs font-body uppercase tracking-wider mb-1">Tu personalidad</p>
          ...
          <p className="text-muted-foreground/80 text-sm font-body mt-1">
            {getSignTrait(data.sunSign.name, "sun")}
          </p>
          <div className="grid grid-cols-2 gap-2 mt-3">
            <div className="flex justify-between items-center p-2.5 bg-muted/30 rounded-lg">
              <span className="text-muted-foreground text-sm font-body">Tu energía</span>
              <span className="text-primary font-medium text-sm font-body">{ELEMENT_FRIENDLY[data.sunSign.element] || data.sunSign.element}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-muted/30 rounded-lg">
              <span className="text-muted-foreground text-sm font-body">Tu impulso</span>
              <span className="text-primary font-medium text-sm font-body">{PLANET_FRIENDLY[data.sunSign.planet] || data.sunSign.planet}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-muted/30 p-4 rounded-xl border border-border">
            <p className="text-muted-foreground text-xs font-body uppercase tracking-wider mb-1">Tus emociones</p>
            <p className="text-foreground text-lg font-display font-semibold">{data.moonSign}</p>
            <p className="text-muted-foreground/70 text-xs font-body mt-1">{getSignTrait(data.moonSign, "moon")}</p>
          </div>
          <div className="bg-muted/30 p-4 rounded-xl border border-border">
            <p className="text-muted-foreground text-xs font-body uppercase tracking-wider mb-1">Cómo te ven</p>
            <p className="text-foreground text-lg font-display font-semibold">{data.ascendant}</p>
            <p className="text-muted-foreground/70 text-xs font-body mt-1">{getSignTrait(data.ascendant, "asc")}</p>
          </div>
        </div>
      </div>
        ...
        >
          ...
          {ritual && (
            <div className="glass-card p-5 text-center">
              ...
              <p className="text-muted-foreground text-xs font-body uppercase tracking-wider mb-2">Práctica Sugerida</p>
          ...
          {amulet && (
            <div className="glass-card p-5 text-center">
              ...
              <p className="text-muted-foreground text-xs font-body uppercase tracking-wider mb-2">Tu Piedra de Protección</p>
      ...
      {data.analysis && (
        ...
        >
          <h3 className="font-display text-lg text-foreground flex items-center gap-2 mb-4 tracking-wide">
            Lo que dice tu perfil sobre ti
          </h3>
