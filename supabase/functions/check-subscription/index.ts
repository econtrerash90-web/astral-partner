...
serve(async (req) => {
  ...
  try {
    ...
    if (userError || !userData.user?.email) {
      logStep("No valid session, returning unsubscribed");
    ...
    if (hasActiveSub) {
      ...
      subscriptionEnd = Number.isFinite(periodEnd) ? new Date(periodEnd * 1000).toISOString() : null;
    ...
    return new Response(JSON.stringify({ error: "subscription_check_failed" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
